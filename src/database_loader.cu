...
/* firma funzione kernel del frame deduplication 
__global__ void dedup_kernel_stub(const uint8_t* d_frames, uint8_t* d_keep, int n,  int bytes_per_frame, int threshold) {} */
...
static void upload_frames(Workspace& ws, const HostChunk& ch) {
  ...
  // Uso default stream (0). Se vuoi in futuro reintrodurre stream, basta cambiare qui.
